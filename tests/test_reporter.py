...
_MTG_WITH_MINUTES = {'date': '2026-03-30', 'status': 'held'}
_MTG_WITHOUT_MINUTES = {'date': '2025-09-15', 'status': 'held'}
_INDEX_WITH_MINUTES = {'2026-03-30': {'minutes': True}}
_INDEX_WITHOUT_MINUTES = {'2025-09-15': {'minutes': False}}
_TOWN_RECORD = {
    'date': '2026-03-30',
...
class TestHasOfficialMinutes:
    def test_true_when_index_shows_minutes(self):
        assert reporter._has_official_minutes(_MTG_WITH_MINUTES, _INDEX_WITH_MINUTES) is True

    def test_false_when_index_shows_no_minutes(self):
        assert reporter._has_official_minutes(_MTG_WITHOUT_MINUTES, _INDEX_WITHOUT_MINUTES) is False

    def test_false_when_date_missing_from_index(self):
        assert reporter._has_official_minutes(_MTG_WITH_MINUTES, {}) is False
...
class TestCompareReport:
    ...
    def test_both_section_shows_shared_date(self):
        report = reporter.compare_report([_MTG_WITH_MINUTES], [_TOWN_RECORD], _INDEX_WITH_MINUTES)
        assert '2026-03-30' in report
        assert 'BOTH' in report.upper()

    def test_only_mtg_section_shows_mtg_only_date(self):
        report = reporter.compare_report([_MTG_WITH_MINUTES], [], _INDEX_WITH_MINUTES)
        assert '2026-03-30' in report
    ...
    def test_summary_counts_correctly(self):
        ...
        report = reporter.compare_report(meetings, town, _INDEX_WITH_MINUTES)
        assert 'SUMMARY' in report

    def test_meetings_without_index_minutes_excluded_from_mtg_set(self):
        report = reporter.compare_report([_MTG_WITHOUT_MINUTES], [], _INDEX_WITHOUT_MINUTES)
